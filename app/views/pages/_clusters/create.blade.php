...
@section('content')
    ...
    <div class="page_content">
      <h3>Cluster</h3>
      <p>
		{{ Form::open(array('route' => 'clusters.store', 'method' => 'post')) }}
		    ...
		    <ul style="list-style:none;">
				...
				<div class="form-group">
				    ...
				    {{ Form::textarea('subtitle_de', null, ['rows' => 3, 'style' => 'vertical-align:top;', 'placeholder' => 'Subtitle [de]']) }}
				    {{ Form::textarea('subtitle_en', null, ['rows' => 3, 'style' => 'vertical-align:top;', 'placeholder' => 'Subtitle [en]']) }}
				</div>
				<div class="form-group" class="nl">
				    <label for="exampleInputEmail1">{{ Form::label('remarks', 'Remarks:') }}</label>
				    {{ Form::textarea('remarks_de', null, ['rows' => 6, 'style' => 'vertical-align:top;', 'placeholder' => 'Remarks [de]']) }}
				    {{ Form::textarea('remarks_en', null, ['rows' => 6, 'style' => 'vertical-align:top;', 'placeholder' => 'Remarks [en]']) }}
				</div>
				<div class="form-group" class="nl">
	        	...
	        	<?php foreach($k_events as $ke): 
	        			 $selected = '';
	        	?>
				        <option value="<?php echo $ke->id;?>" <?php echo $selected;?>><?php echo $ke->id . ' - '.$ke->title_de; ?> </option>
	        	<?php endforeach; ?>
				    </select>
				</div>
