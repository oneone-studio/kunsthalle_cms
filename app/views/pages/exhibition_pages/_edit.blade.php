		...
		<div style="list-style:none;">
			{{ Form::model($page, array('route' => array('exb-pages.save', $page->id))) }}		  
				...
				<div class="form-group" class="nl">
				    <label for="title">{{ Form::label('tag', 'Tags:') }}</label>
				    <select name="tags[]" id="tags" class="chosen-select" multiple style="width:650px;">
				      @foreach($rel_tags as $t) 
				      	 <option value="{{$t->id}}" selected="selected">{{$t->tag_de}}</option>
				      @endforeach
				      @foreach($tags as $t) 
				      	 <option value="{{$t->id}}" >{{$t->tag_de}}</option>
				      @endforeach
				    </select>
				</div>
			...
			{{ Form::close() }}		
			    ...
			    ?>   
				<div id="sponsors_block" class="form-group" style="margin-top:2px; {{$display}}">
				...
				<div id="downloads_block" class="form-group" style="margin-top:2px; {{$display}}">
				  <form id="dl_protection_form" method="post" action="/save-exb-dl-protection" enctype="multipart/form-data">
				    <div style="clear:both;"></div>
				...
				<div id="cp_block" class="form-group" style="margin-top:20px;">
			    	...
			    	<div style="width:100%; position:relative; top:25px; margin-bottom: 15px; clear:both;">
						    ...
						    <div style="width:16px; text-align: center; display:inline; padding:0px 4px; float:left; color:#888;">|</div>
						    <a href="javascript:toggleInput('slider')" class="form-link"><img src="/files/sliders.png" style="width:22px;height:22px;float:left;margin-right:3px;"> </a>
						    <div style="width:16px; text-align: center; display:inline; padding:0px 4px; float:left; color:#888;">|</div>
					    		...
					    		<li id="slider_pane_{{$slider->id}}" class="no-display" style="padding:10px 0px;">
					    		  ...
					    		  <div id="slider_image_form_blk_<?php echo $slider->id;?>" style="width:100%; background:#f7f7f7; display:none; clear:both;">
								   <form id="slider_image_form_<?php echo $slider->id;?>" enctype="multipart/form-data" method="post" action="">
								    ...
								    {{ Form::hidden('slider_image_id', 0, ['id' => 'slider_image_id'])}}
...
function showPageSliderImagePreview(id) {
	...
	$.ajax({
	    type: 'POST',
	    url: '/exb-page-slider-image-preview',
	    data: formData,
	    dataType: 'json',
	    processData: false,
        contentType: false,
...
function updatePageSliderImage(event, slider_id, id) {
	...
	$.ajax({
	    type: 'POST',
	    url: '/update-exb-page-slider-image',
	    data: formData,
	    dataType: 'json',
	    processData: false,
        contentType: false,
