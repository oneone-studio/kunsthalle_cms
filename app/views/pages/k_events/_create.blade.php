    ...
    <div class="page_content">
      ...
      <p>
		...
		{{ Form::open(array('action' => 'KEventsController@store', 'method' => 'post', 'class' => 'form-inline')) }}
			...
			<div class="form-group nl" ng-click="getExbs()" style="margin-top:10px;">
			    <label for="cluster"><?php echo Form::label('clusters', 'Clusters'); ?></label>
			    <select name="clusters[]" data-placeholder="Choose an event cluster..." class="chosen-select" multiple="multiple" style="width:300px;" tabindex="4">
			        <option value=""></option>
        	<?php foreach($clusters as $cl): 
        			 $selected = '';
        	?>
			        <option value="<?php echo $cl->id;?>" <?php echo $selected;?> ><?php echo $cl->title_de . ' ['.$cl->cluster_type .']'; ?> </option>
        	<?php endforeach; ?>
			    </select>
			</div>
...
<!-- 
			...
			<div class="form-group nl" style="padding:10px 0px;">
			    ...
			    <div id="image_pane" style="width:10%; float:left; display:inline;">
			      ...
			      background-size:70px 100px; border:1px dashed #e3e3e3;"><img id="preview" src="#" style="max-width:70px; max-height:100px; float:left;"></div>
			    </div>
			      <div style="width:20px; display:inline; margin-left:10px;"><a id="deleteEventImageLnk" class="glyphicon glyphicon-trash" 
			      style="cursor:pointer; margin-top:80px;"></a></div>
			</div>
 -->
			<!-- <div class="form-group nl">
				...
				<div style="width:400px; display:inline-block; margin-left:4px; margin-bottom:10px;">    
			    	...
			    	<div style="width:100%; float:left; margin-left:0px; margin-top:10px; margin-bottom:5px;">    
				    <div style="width:160px; height:auto; float:left;">
					    <div id="event_day_div" style="width:150px; height:30px; float:left;"> -->
...
<!-- 					      <select name="event_day" id="event_day" data-placeholder="Choose a day" class="chosen-select">
					    ...
					    <ul style="width:160px; float:left; list-style:none; padding-top:10px; margin-left:3px;">
					       <li style="margin-left:0;position:relative; height:20px;"><?php echo Form::radio('event_day_repeat', 'every', false, ['class' => 'event_repeat_opt', 'style' => 'width:15px; position:relative; left:0;']); ?> <div style="width:100%; display:inline; position:relative; top:2px; margin-left:4px;">Every</div></li>
					       <li style="margin-left:0; position:relative; height:20px;"><?php echo Form::radio('event_day_repeat', 'every first', false, ['class' => 'event_repeat_opt', 'style' => 'width:15px; position:relative; left:0;']); ?> <div style="width:100%; display:inline; position:relative; top:2px; margin-left:4px;"> Every first</div></li>
					       <li style="height:20px;"><?php echo Form::radio('event_day_repeat', 'every second', false, ['class' => 'event_repeat_opt', 'style' => 'width:15px;  position:relative; left:0;']); ?> <div style="width:100%; display:inline; position:relative; top:2px; margin-left:4px;"> Every second</div></li>
					       <li style="height:20px;"><?php echo Form::radio('event_day_repeat', 'every last', false, ['class' => 'event_repeat_opt', 'style' => 'width:15px;  position:relative; left:0;']); ?> <div style="width:100%; display:inline; position:relative; top:2px; margin-left:4px;"> Every last</div></li>
					       <li style="height:20px;"><?php echo Form::radio('event_day_repeat', 'bi-weekly', false, ['class' => 'event_repeat_opt', 'style' => 'width:15px;  position:relative; left:0;']); ?> <div style="width:100%; display:inline; position:relative; top:2px; margin-left:4px;"> bi-weekly, on a</div></li>
					    </ul>
					</div>
				</div>
			</div> -->
...
<!-- 			<div class="form-group nl" style="padding-bottom:10px;">
			    <label for="exampleInputEmail1"><?php echo Form::label('event_dates_ch', 'Single Dates'); ?></label>
			    <?php 
			    ...
			    <?php echo Form::checkbox('event_dates_ch', null, false, ['style' => 'vertical-align:top; margin-top:-1px; margin-right:5px;', 'onclick' => 'toggleEventRepeat(this)']); ?>

			    <?php echo Form::label('event_date', 'Event Date', ['style' => 'width:80px;']); ?> <?php echo Form::text('event_date', null, ['style' => 'width:100px;']); ?>

			    <input type="button" value="Save" onclick="addSingleDate()">
			    <input type="hidden" name="single_dates" id="single_dates">
			    <br>
			   
			    </div>
			</div>
			<div id="event_dates_pane" class="form-group nl" style="width:77%; float:right; clear:both; padding:10px; border-radius:5px; 
				border:0px solid #9f9f9f; margin-bottom:25px;">
			</div> 
			...
			<div class="form-group nl" style="margin:10px 0px;">
				...
				<div style="width:500px;display:inline-block;margin-left:0px;">
				    <select name="page_url" id="page_url" style="width:500px;" onchange="showPageLink(this, '{{$k_event->id}}')" class="chosen-select">
				    <option value="0">-- Select Page -- </option>
				    @if(isset($page_links))
		    	    	@foreach($page_links['page_sections'] as $sec)
		    	    		<option disabled>{{$sec['title']}} [section] </option>
		    	    	...
		    	    	@if(count($page_links['exb_pages']) > 0)
		    	    		<option disabled>Exhibition Pages</option>
